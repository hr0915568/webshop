    ...
    stages {
        ...
        stage('Deploy') {
            steps {
                ...
                sh 'kill $(ps aux | grep \'webshop-1.0-SNAPSHOT -Dplay.http.secret.key=hrwebshop\' | awk \'{print $2}\')'
                ...
                sh 'ssh -t docker@192.168.1.7 \'webshop-1.0-SNAPSHOT/bin/webshop -Dplay.http.secret.key=hrwebshop -Dplay.evolutions.db.default.autoApply=true & \''
            }
        }
    }
}
